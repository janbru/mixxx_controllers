...
HerculesAir.rec= function(midino, control, value, status, group) {
    if (value==0x7f) {
        HerculesAir.enableRec = !HerculesAir.enableRec;
		script.toggleControl(group, "toggle_recording")
        if (HerculesAir.enableRec) {
            midi.sendShortMsg(status,control, 0x7f);
        } else {
            midi.sendShortMsg(status,control, 0x0);
        }
